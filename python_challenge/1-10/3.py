...
found = False # flag indicating if all letters are capital
buf = string.rsplit(source) # buf now is a list with the lines without the EoL characters
copyOfSrc="" # this will become the string representation of bug
result = ""

for b in buf:
    copyOfSrc += b
length = len(copyOfSrc)

index = -1
for letter in copyOfSrc:
    index += 1
        
    if ascii_lowercase.find(letter) != -1: # it is a lowercase
        if ascii_uppercase.find(copyOfSrc[(index-4)%length]) != -1  or ascii_uppercase.find(copyOfSrc[(index+4)%length]) != -1: # needs EXACTLY 3 on each side
            continue
        
        found = True
        #index = copyOfSrc.find(letter) # this finds only the first occurrence!
        candidate = copyOfSrc[index-3 : index] + copyOfSrc[index+1 : index+4]
        # if we need to wrap around the text
#        candidate += copyOfSrc[(index-80)%length] + copyOfSrc[(index-2*80)%length] + copyOfSrc[(index-3*80)%length]+ copyOfSrc[(index+80)%length] + copyOfSrc[(index+2*80)%length] + copyOfSrc[(index+3*80)%length]

        if len(candidate) != 6:
            continue
        
        for l in candidate:
            if ascii_uppercase.find(l) == -1: # it is an uppercase
                found = False
                break
        if found:
            result += letter
